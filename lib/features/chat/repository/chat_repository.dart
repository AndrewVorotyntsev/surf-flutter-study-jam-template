...
class ChatRepository implements IChatRepository {
  ...
  @override
  Future<Iterable<ChatMessageDto>> getMessages({int? id}) async {
  ...
  Future<Iterable<ChatMessageDto>> _fetchAllMessages({int? id}) async {
    ...
    while (!isLimitBroken) {
      final batch = await _studyJamClient.getMessages(
          chatId: id, lastMessageId: lastMessageId, limit: 10000);
    ...
    return messages
        .map((sjMessageDto) => sjMessageDto.geopoint == null
                ...
                : sjMessageDto.images != null
                    // Сообщение с геолокацией и изображением
